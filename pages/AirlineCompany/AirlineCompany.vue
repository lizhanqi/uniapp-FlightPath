	...
	<view    class="uni-flex uni-column app-bg"  >  
		 <view  class="app-card" style="height: 100%;  ">
			 ...
			 <view   v-if="inSearch" >
			   ...
			   "  @click="onItem(item)">{{item.carrierNameCn?item.carrierNameCn: item.carrierName   }}   </text>
				</block>
			 </view>
			<view>
		...
		methods: {
				...
				matchInput(text){  
					...
					this.allCity.forEach(( item, index) => {
						...
						try
						   { 
								 var cityCode=	item.carrierCode ;
								 var cityName=	item.carrierName ;
								 var cityNameCn=	item.carrierNameCn ;
								 var cityNamePinyin=	item.carrierNamePinyin ;
